 ...
 * Run with: npx ts-node scripts/add-demo-data.ts
 */
    ...
    for (const [location, facilities] of Object.entries(locations)) {
      ...
      await prisma.facility.createMany({
        data: facilities.map((name) => ({
          name,
          system: system as "NDWH" | "CBS",
          location: location as "Kakamega" | "Vihiga" | "Nyamira" | "Kisumu",
          isMaster: true,
        })),
        skipDuplicates: true,
      })

      // Add some reported facilities (partial reporting)
      const reportedCount = Math.floor(facilities.length * 0.6) // 60% reported
